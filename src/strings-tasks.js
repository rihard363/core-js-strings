...
function removeFirstOccurrences(str, value) {
  ...
  let result = str.slice(0, x) + str.slice(x + value.length);
  if (x < 0) {
    result = str;
  }
  return result;
...
function encodeToRot13(str) {
  ...
  const adcCod = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  const s = str.split('');
  for (let i = 0; i < s.length; i += 1) {
    const elLength = s[i].length;
    if (s[i] !== ' ' && s[i] !== ',' && s[i] !== '!' && s[i] !== '?') {
      for (let j = 0; j < elLength; j += 1) {
        const count = s[i][j];
        const index = abc.indexOf(count);
        s[i] = adcCod[index];
      }
    }
  }
